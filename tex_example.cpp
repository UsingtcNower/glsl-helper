...
//<<<<<<<<<<<<<<<<<<<<<<< myInit >>>>>>>>>>>>>>>>>>>>
 void myInit(void)
 {
	...
	cv::Mat thumb_img = cv::imread("thumb.png");
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, thumb_img.cols, thumb_img.rows, 0, GL_RGB, GL_UNSIGNED_BYTE, thumb_img.data);
...
void main(int argc, char ** argv)
{
	...
	glutCreateWindow("opengl tex example"); // open the screen window
